...
A **timed hold** is the in-between state. `POST /holds` claims a supplier's
time — one session, a cherry-picked subset of a slate, or a whole slate —
so nobody else can book it (or even see it as available) while your customer
pays. On payment success you convert the hold into real bookings with
`POST /holds/{reservationId}/convert`; on abandonment you release it, or you
simply let it lapse at `heldUntil`.

Holding a **single session** needs no package and no slate — just the
supplier and a UTC window. And the rule that catches everyone: **if you
hold, you convert.** The direct booking routes never consume a hold, not
even your own — they return `409 SLOT_HELD` with the `reservationId` in the
body. See [Hold slots during checkout](/guides/hold-slots-for-checkout) for
the full lifecycle.
