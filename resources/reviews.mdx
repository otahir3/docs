...
```
configured:  https://example.com/review/{bookingId}
sent:        https://example.com/review/bk_123?bookingId=bk_123&role=client

configured:  https://example.com/feedback?src=mb
sent:        https://example.com/feedback?src=mb&bookingId=bk_123&role=supplier&rating=4
```

Three more tokens, `{bookingLocale}`, `{clientLocale}` and `{supplierLocale}`, are substituted
the same way and each falls back to the project's default locale. See
[Emails](/concepts/emails#where-the-rating-links-go) for what each one resolves to.
