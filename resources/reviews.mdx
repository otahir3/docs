...
sequenceDiagram
    ...
    M-->>C: rating email, linking to your page with a bookingId
    C->>P: opens the page
...
Responses carry `supplierName`, `supplierEmail`, `clientName` and `clientEmail` alongside the ids,
so a page can render a name without a second call. They are best-effort: a counterparty whose
profile cannot be resolved keeps its bare id and the name fields are simply absent.
...
MarketBox emails a client after a booking asking for a rating, and follows up if none arrives.
**A review linked to a booking stops those follow-ups for it.** The booking-scoped route links it
for you. A review posted through the project-level route without a `bookingId` does not, and the
client keeps being asked to rate something they have already rated.
...
| `supplierName`, `supplierEmail`, `clientName`, `clientEmail` | Best-effort display fields. |
