...
description: Reserve a supplier's time while a client pays — hold a single session or a whole package slate, then convert the hold into bookings.
---
...
what turns "the client is on the payment page" into a state the booking
...
needs no package, no slate, and no `PACKAGE` business model —
...
2. **Create the order and take payment.** An order with a
   ...
   a multi-session hold) is what funds the bookings. Charge the client
...
If the client abandons, `POST /holds/{reservationId}/release` frees the
time immediately.
...
while your client was paying), so whatever the booking will need, the
...
export async function POST(request: Request) {
  ...
  const res = await fetch(
    `${process.env.MARKETBOX_API_URL}/v1/projects/${process.env.MARKETBOX_PROJECT_ID}/holds`,
    {
      method: 'POST',
      headers: {
...
<Step title="Keep the hold alive while the client pays">
...
export async function POST(request: Request) {
  ...
  const res = await fetch(
    `${process.env.MARKETBOX_API_URL}/v1/projects/${process.env.MARKETBOX_PROJECT_ID}/holds/${reservationId}/convert`,
    {
      method: 'POST',
      headers: {
        'x-api-key': process.env.MARKETBOX_API_KEY!, // server-only
...
<Note>
**Convert is what sends the emails — holding sends nothing.** A hold has no
booking attached to it, so nothing is mailed while the client is still on the
payment page. The moment convert succeeds, MarketBox sends the client their
booking confirmation and each assigned supplier their new-booking notice, and
schedules the reminder and the rating requests that follow.

So **don't send your own booking confirmation** from your webhook — the client
will already have one. A multi-session package converts into a single
confirmation covering every session, not one per session. A replayed convert
does not re-send anything.

See [Emails](/concepts/emails) for the full set and for the two things you can
configure: the from name and the reply-to address.
</Note>
...
<Step title="Release, if the client abandons">
...
export async function POST(request: Request) {
  ...
  const res = await fetch(
    `${process.env.MARKETBOX_API_URL}/v1/projects/${process.env.MARKETBOX_PROJECT_ID}/holds/${reservationId}/release`,
    {
      method: 'POST',
...
<Step title="Create the order and charge the client">
...
- **It's not yours** (another client is mid-checkout) → the time is
...
| `409` | `SLOT_HELD` / `SLOT_BOOKED` | Someone else claimed the time while your client was paying (only possible after your hold lapsed). | The time is gone. Refund or re-book onto a fresh slot. |
