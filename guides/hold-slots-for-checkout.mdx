...
for the service and offering being sold, then your usual Stripe flow. The
address you sent to `/holds` is the *session's* address — it is not the
order's billing address and is not sent again.
