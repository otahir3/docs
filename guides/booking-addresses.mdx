...
<Tip>
**Reuse a client's saved address.** Each entry in a client profile's
`addresses[]` now carries `latitude` and `longitude` as well as the structured
fields. That is everything a `TRAVEL_ZONE` booking needs, so you can book a
visit to a saved address without geocoding it again. Either coordinate can be
`null` when the address was saved without one; geocode those before booking.
</Tip>
...
responsibility (e.g. via a geocoding API), not MarketBox's.
</Warning>
