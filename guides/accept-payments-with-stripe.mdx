...
description: Charge customers for orders in your booking flow using Stripe, from connecting a business's Stripe account to confirming a card with the Payment Element and finalizing the order.
---

This guide walks you through taking real payments in a custom booking flow. A
customer books, you charge their card with Stripe, and the order is marked
...
customer's browser to Stripe** using Stripe's Payment Element, so your servers
...
Before you can charge, a project admin connects the business's Stripe account in
the [MarketBox dashboard](https://app.marketbox.io), under **Payments**. This is
a one-time action per business, done through Stripe's hosted onboarding.
...
charges. Until then, creating a PaymentIntent returns
`409 PROJECT_NOT_CHARGE_READY`. The business owner completes this in their own
Stripe dashboard; the platform cannot fill it in for them.
</Warning>
</Step>
...
MARKETBOX_API_KEY=...
API_BASE_URL=https://api.marketbox.io
PROJECT_ID=prj_...
```
...
export async function POST(request: Request) {
  ...
  const res = await fetch(
    `${process.env.API_BASE_URL}/v1/projects/${process.env.PROJECT_ID}/orders/${orderId}/payment-intent`,
    {
      method: 'POST',
      headers: {
...
export async function GET(
  ...
  const res = await fetch(
    `${process.env.API_BASE_URL}/v1/projects/${process.env.PROJECT_ID}/orders/${orderId}`,
...
customer something like *"You last paid with Visa ending 4242"* without building
your own card store.
...
const res = await fetch(
  `${API_BASE_URL}/v1/projects/${PROJECT_ID}/clients/me`,
  {
    method: 'POST',
    headers: {
...
cards a customer actually has. It is written from Stripe's `charge.succeeded`
...
you at SAQ-A. `lastPayment` tells you what the customer *used* last; it does not
let you re-use it.
</Note>
