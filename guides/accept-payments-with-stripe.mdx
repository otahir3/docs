...
to. Asking `POST .../orders/{orderId}/status` for `PAID` is refused with
`409 ORDER_NOT_SETTLED`.

<Note>
**The card is saved to the client.** When the order's client has an email on
file, the PaymentIntent is created with `setup_future_usage: off_session`, so
the card is saved to that client's record in Stripe for future bookings. Tell
the client at checkout that their card will be saved. An intent reused from an
earlier call is updated to save the card too, provided it has not been
confirmed yet.
</Note>
...
you can see both the money in and the credits drawn down. A cancelled session
whose credit goes back to the package adds a `CREDIT_REVERSAL` that points at
its redemption through `relatedTransactionId`. Credit actually used is
redemptions minus reversals. Neither is money: both have
`source: "PACKAGE_CREDIT"`.
