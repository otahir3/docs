...
<Note>
**Prerequisite: the service needs an active `FLEXIBLE_PACKAGE` business
...
stamped `businessModelType: "FLEXIBLE_PACKAGE"` with `packageOrderItemId`
...
A package booking is three calls, always in this order:
...
2. **Create the order.** `POST /orders` with one `PACKAGE` item is what makes
   ...
   from the package.
3. **Book the slate.** `POST /orders/{orderId}/package-bookings` with just
...
export async function POST(request: Request) {
  ...
  const res = await fetch(
    `${process.env.API_BASE_URL}/v1/projects/${process.env.PROJECT_ID}/availability/package-slate`,
    {
      method: 'POST',
      headers: {
...
them from the package definition.
...
```json
{
  "data": {
    "order": {
      ...
      "status": "DRAFT",
      "total": { "amount": 32000, "currency": "CAD" }
    },
    "items": [
      {
...
with `orderId` and the `slateId` from Step 1.
</Step>

<Step title="Book the slate">
Book all N sessions in one call by passing the `orderItemId` from Step 2 and
the `slateId` from Step 1 — no session details required. Provide **exactly
...
export async function POST(
  ...
  const res = await fetch(
    `${process.env.API_BASE_URL}/v1/projects/${process.env.PROJECT_ID}/orders/${orderId}/package-bookings`,
    {
      method: 'POST',
      headers: {
        'x-api-key': process.env.MARKETBOX_API_KEY!, // server-only
...
(Step 1) but **0-based** in this `created[]` response. Slot `sessionIndex: 1`
...
- **Transcribe explicit sessions.** Instead of sending `slateId`, copy every
  ...
  locations).
- **Holds → convert.** Reserve the slate's slots while the customer pays,
  then convert the hold to confirmed bookings on payment success — the
  pattern you want for a timed checkout.

Both are covered in depth in the
...
| More than 20 sessions with `atomicity: "all_or_nothing"` | `PACKAGE_SLATE_TOO_LARGE_FOR_ATOMIC` | `422` | Use `atomicity: "best_effort"` (requires the project to enable `allowPartialPackages`), or book a smaller slate. |
...
| `orderItemId` isn't a `PACKAGE` item | `NOT_A_PACKAGE_ITEM` | `400` | Pass the `orderItemId` of the `PACKAGE` item from Step 2, not a `SINGLE_SESSION` item. |
