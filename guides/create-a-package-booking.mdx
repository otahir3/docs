...
1. **Find the package.** `GET /services/{serviceId}/packages?status=ACTIVE`
   ...
   the call a storefront makes before the customer has picked an option. It
...
the customer has chosen an option, and each package carries its `offeringId`
...
`FLEXIBLE_PACKAGE` **business model** is only a flag on the service meaning
...
Take the `packageId` of the bundle the customer picks into the next step.
</Step>
