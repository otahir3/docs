...
copy-pasted `-04:00` will be wrong off-season. `timezone` must be an **IANA
identifier** (`America/Toronto`, never `EST`). `rrule` must include a
**bounded, positive `COUNT`** (e.g. `COUNT=8`) — an unbounded rrule almost
always returns nothing bookable. All three are enforced request-shape
validation (`400`), not just convention.
</Warning>
...
`matchScore` is the mean of `matchComponents` — here `1`, since session one
lands on the window's first day and the plan runs at exactly the requested
cadence. See [How well a slate matches](/guides/availability-and-slates#how-well-a-slate-matches-matchscore-and-matchcomponents)
for the full breakdown and how ranking works.
...
| `timezone` or `bookingTz` is an abbreviation or offset (`EST`, `GMT+5`) rather than an IANA identifier | `VALIDATION_ERROR` | `400` | Send an IANA zone, e.g. `America/Toronto`. See [Timezones](/concepts/timezones#valid-timezones). |
