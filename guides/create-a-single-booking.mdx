...
reserve this supplier's time while the client pays, hold it first:
...
package, no slate. Each held interval takes the **same location fields as
the booking body below** (`bookingTz` and `locationType` are required, and
`locationType` decides the rest) — the hold is what convert rebuilds the
booking from. See
...
from the offering itself. You **do** need to tell the order where this
session happens: this example books a `TRAVEL_ZONE` session (Step 3 below),
so it passes the client's service `address` — the same one the booking call
uses. Booking a `PHYSICAL` session instead? Pass that Location's
`locationId` here rather than `address`. See
[Regional pricing](/concepts/regional-pricing) for the full priority order
(`reservationId` → `locationId` → `address`) and why it matters.
  ...
  const res = await fetch(
    `${process.env.MARKETBOX_API_URL}/v1/projects/${process.env.MARKETBOX_PROJECT_ID}/orders`,
    {
      ...
      body: JSON.stringify({
        clientId,
        currency: 'CAD',
        address: {                      // the session's address — selects the CAD price
...
the offering's `DEFAULT` rule** — which may be a different currency from the
regional price you displayed. If you get `Item currency X does not match
order currency Y`, that's what happened: you told the order what currency
you quoted, but not *where* the session happens, so it couldn't resolve the
same regional rule. See [Regional pricing](/concepts/regional-pricing).
</Warning>
...
export async function GET(
  ...
  const res = await fetch(
    `${process.env.MARKETBOX_API_URL}/v1/projects/${process.env.MARKETBOX_PROJECT_ID}/orders/${orderId}/aggregate`,
...
export async function POST(request: Request) {
  ...
  const res = await fetch(
    `${process.env.MARKETBOX_API_URL}/v1/projects/${process.env.MARKETBOX_PROJECT_ID}/bookings`,
    {
      method: 'POST',
      headers: {
        'x-api-key': process.env.MARKETBOX_API_KEY!, // server-only
