...
| `startLocal` / `endLocal` | The same times rendered in the request `timezone` (ISO-8601 with that zone's offset at that instant). **For display** — don't re-parse them for the booking call, use the UTC pair. |
...
| `timezone` | No | IANA zone (`America/Toronto`) for day bucketing + `*Local` fields. Defaults to the project timezone. Abbreviations and offsets (`EST`, `GMT+5`) are a `400`. |
